...
def clean_earned_premiun(file_path):
    """
    ...
    df_clean["Accident Month"] = pd.to_datetime(df_clean["Accident Month"])
    df_clean = df_clean.set_index("Accident Month")

    current_month = (
        "2025-06-01 00:00:00"  # pd.to_datetime(datetime.now().strftime('%Y-%m'))
    )
    df_clean = df_clean[df_clean.index <= current_month]
